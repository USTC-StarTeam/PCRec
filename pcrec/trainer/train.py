...
def train_fn(
    ...
    elif loss_module == "CELoss":
        loss_debug_str = loss_debug_str[:-4]
        #assert temperature == 1.0
        ar_loss = CELoss(temperature=1.0, model=model)
    ...
    for epoch in range(num_epochs):
        ...
        for row in iter(train_data_loader):
            ...
            #mse_loss = F.mse_loss(
            #    seq_embeddings[:, :-1, :], input_embeddings[:, 1:, :], reduction='mean'
            #) * seq_features.past_lengths.shape[0]
            #loss = (1 - weight_mse) * loss + weight_mse * mse_loss
            if rank == 0:
