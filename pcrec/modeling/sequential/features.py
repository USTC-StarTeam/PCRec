...
def movielens_seq_features_from_row(
    ...
    if max_output_length > 0:
        ...
        '''if train_flag == True:
            ...
            max_sequence_length = N - max_output_length
            historical_lengths_full = torch.full((B,), max_sequence_length)

            result = torch.zeros_like(historical_ids, device=device)
            meaningful_parts = [historical_ids[i, :historical_lengths[i]] for i in range(B)]
            full_repeats = max_sequence_length // historical_lengths
            remainder = max_sequence_length % historical_lengths
            for i in range(B):
                meaningful_part = meaningful_parts[i]
                full_repeat_part = meaningful_part.repeat(full_repeats[i].item())
                remainder_part = meaningful_part[:remainder[i].item()]

                result[i, :full_repeat_part.size(0)] = full_repeat_part
                result[i, full_repeat_part.size(0):max_sequence_length] = remainder_part

                result[i, max_sequence_length:] = historical_ids[i, max_sequence_length:]
            
            historical_ids_full = result
        else:
            historical_ids_full = historical_ids
            historical_lengths_full = historical_lengths '''


    features = SequentialFeatures(
        #past_lengths=historical_lengths,
        past_lengths=historical_lengths,  #_full.to(device),
        past_ids=historical_ids,    #_full.to(device),
        past_embeddings=None,
        past_payloads={
            "timestamps": historical_timestamps.to(device),
            "ratings": historical_ratings.to(device),
        },
    )
