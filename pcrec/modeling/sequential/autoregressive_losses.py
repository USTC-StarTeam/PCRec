...
class CELoss(AutoregressiveLoss):
    def __init__(
        self,
        temperature: float,
        model: SimilarityModule,
    ...
    def jagged_forward(
        ...
        # Calculate logits for all classes (assuming model returns logits directly)
        logits = self._model.interaction(
            input_embeddings=output_embeddings,  # [B, D] = [N', D]
            target_ids=supervision_ids,  # [N']
            target_embeddings=supervision_embeddings,  # [N', D]
        )[0] / self._temperature  # [N', num_classes]

        # Compute Cross Entropy Loss
        loss = F.cross_entropy(
            input=logits,
            target=supervision_ids,
            weight=supervision_weights,
            reduction='none'
        )

        # Apply supervision weights
        weighted_losses = loss * supervision_weights
