...
"""
Implements pcrec (Hierarchical Sequential Transduction Unit) in
...
pcrecCacheState = Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]


def _pcrec_attention_maybe_from_cache(
    num_heads: int,
    attention_dim: int,
    linear_dim: int,
    q: torch.Tensor,
    ...
    if delta_x_offsets is not None:
        ...
        padded_q = torch.ops.fbgemm.jagged_to_padded_dense( #torch.Size([128, 211, 50])
...
class SequentialTransductionUnitJagged(torch.nn.Module):
    def __init__(
        ...
        dim_o = linear_hidden_dim * num_heads * (3 if concat_ua else 1)
        ...
        self.learnable_lr_weight = torch.nn.Parameter(torch.randn(embedding_dim, embedding_dim))
        self.learnable_lr_bias = torch.nn.Parameter(torch.randn(embedding_dim))
        #self.sequenceMixer = torch.nn.Linear(in_features=max_length, out_features=max_length, bias=False)
        #PreNormResidual(self.hidden_size, FeedForward(self.max_seq_length, expansion_factor, self.hidden_dropout_prob, chan_first))
        #self.W1 = torch.nn.Parameter(torch.normal(0, 0.02, size=(max_length,  max_length)))
        self.W1 = torch.nn.Parameter(torch.normal(mean=0.0, std=0.02, size=(211, 211)))
        self.linear_layer = torch.nn.Linear(embedding_dim,  embedding_dim)  # Assuming original dimension is 50
        #self.linear_expand = torch.nn.Linear(embedding_dim*2, embedding_dim)
        self.activation_f='identity'
        self.alpha = alpha
    ...
    def get_eta_ttt(self, X, mini_batch_size, mini_batch_step_offset=0):
    ...
    def get_eta(self, x):
        lr = torch.matmul(x, self.learnable_lr_weight) + self.learnable_lr_bias
        lr = torch.sigmoid(lr)

        return lr
    
    def get_predict(self, x, activation):
        ...
        x_ac = x_ac.transpose(1, 2)
        y = torch.matmul(x_ac, self.W1) 
        y = y.transpose(1, 2)
        return y
    
    def get_predict_error(self, predicted_, new_outputs_padded, past_lengths):
        error = (predicted_ - new_outputs_padded) ** 2
        batch_size, seq_len, hidden_size = predicted_.size()

        mask = torch.zeros(batch_size, seq_len, dtype=torch.float32, device=predicted_.device)

        for i in range(batch_size):
            mask[i, :past_lengths[i]] = 1

        mask = mask.unsqueeze(-1).expand(-1, -1, hidden_size)
        masked_error = error * mask
        predict_error = masked_error.sum(dim=(1, 2)) / past_lengths.float().unsqueeze(-1)
    ...
    def get_delta_s(
        ...
        if activation == 'Sigmoid':
            x_ac = F.sigmoid(x)
            f_derivative = x_ac * (1 - x_ac)            
        elif activation == 'Tanh':
            f_derivative = 1 - F.tanh(x) ** 2
            x_ac = F.tanh(x)
        elif activation == 'Softplus':
            f_derivative = F.sigmoid(x)
            x_ac = F.softplus(x)
        elif activation == 'ReLU':
            f_derivative = (x > 0).float()
            x_ac = F.relu(x)
        
        W_T = self.W1.T
        u_x_deri = torch.matmul(W_T, f_derivative)
        if layer_index == 0:
            delta_s = - learning_rate * error_l
        elif layer_index == self._num_layer - 1:
            delta_s = learning_rate * u_x_deri * error_l_1 
        else:
            delta_s = learning_rate * (u_x_deri * error_l_1 - error_l)

        delta_W1 = self.alpha * error_l * x_ac
        
        return delta_s, delta_W1  #(B, S, H)
    ...
    def forward(  # pyre-ignore [3]
        ...
        cache: Optional[pcrecCacheState] = None,
        ...
        normed_x = self._norm_input(x)  #([13166, 50])
        normed_x = self.linear_layer(normed_x)
        lr = self.get_eta(normed_x)
        normed_x_padded = torch.ops.fbgemm.jagged_to_padded_dense( #torch.Size([128, 211, 50])
            values=normed_x, offsets=[x_offsets], max_lengths=[n], padding_value=0.0
        )
        predicted_ = self.get_predict(normed_x_padded, self.activation_f)
        predict_error_l_0 = self.get_predict_error(predicted_, normed_x_padded, past_lengths)

        #token_eta, ttt_lr_eta = self.get_eta(normed_x, self.mini_batch_size)

        if self._linear_config == "uvqk":
            batched_mm_output = torch.mm(normed_x, self._uvqk)
            #batched_mm_output = batched_mm_output + self.bias
            if self._linear_activation == "silu":
                batched_mm_output = F.silu(batched_mm_output)  #torch.Size([13554, 200])
            elif self._linear_activation == "none":
                batched_mm_output = batched_mm_output
            u, v, q, k = torch.split(  #torch.Size([13554, 50])
                batched_mm_output,
                [
        ...
        B: int = x_offsets.size(0) - 1
        if self._normalization == "rel_bias" or self._normalization == "pcrec_rel_bias":
            assert self._rel_attn_bias is not None
            attn_output, padded_q, padded_k = _pcrec_attention_maybe_from_cache(
        ...
        new_outputs_padded = torch.ops.fbgemm.jagged_to_padded_dense(new_outputs, [x_offsets], [n]) #shape = torch.Size([128, 211, 50])
        predict_error_l = self.get_predict_error(predicted_, new_outputs_padded, past_lengths)
        
        delta_s, delta_W1 = self.get_delta_s(
            x = new_outputs_padded,
            learning_rate = lr, 
            error_l_1 = predict_error_l_0, 
            error_l = predict_error_l, 
            layer_index = layer_index, 
            activation = self.activation_f,
        )   #(B, S, H)

        outputs_padded = new_outputs_padded + delta_s

        with torch.no_grad():
            self.W1 += delta_W1       

        outputs = torch.ops.fbgemm.dense_to_jagged(
                outputs_padded,
                [x_offsets],
            )[0]
        
        if delta_x_offsets is not None:
            outputs = cached_outputs.index_copy_(
                dim=0, index=delta_x_offsets[0], source=outputs
            )

        if return_cache_states and delta_x_offsets is None:
            v = v.contiguous()

        return outputs, (v, padded_q, padded_k, outputs)


class PCRecJagged(torch.nn.Module):
    ...
    def jagged_forward(
        ...
        invalid_attn_mask: torch.Tensor,
        delta_x_offsets: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        cache: Optional[List[pcrecCacheState]] = None,
        return_cache_states: bool = False,
    ) -> Tuple[torch.Tensor, List[pcrecCacheState]]:
        """
        Args:
            x: (\sum_i N_i, D) x float
            x_offsets: (B + 1) x int32
        ...
        Returns:
            x' = f(x), (\sum_i N_i, D) x float
        """
        cache_states: List[pcrecCacheState] = []
...
class pcrec(SequentialEncoderWithLearnedSimilarityModule):
    """
    Implements pcrec (Hierarchical Sequential Transduction Unit) in
    Actions Speak Louder than Words: Trillion-Parameter Sequential Transducers for Generative Recommendations,
    ...
    def __init__(
        ...
        self.position_embedding = torch.nn.Parameter(torch.zeros(1, self._max_sequence_length + 11, self._item_embedding_dim))
        torch.nn.init.normal_(self.position_embedding, mean=0, std=0.02)
        self._pcrec = PCRecJagged(
            modules=[
                SequentialTransductionUnitJagged(
                    num_layer=self._num_blocks,
    ...
    def reset_params(self) -> None:
        for name, params in self.named_parameters():
            if ("_pcrec" in name) or ("_embedding_module" in name):
                if self._verbose:
    ...
    def debug_str(self) -> str:
        debug_str = (
            f"pcrec-b{self._num_blocks}-h{self._num_heads}-dqk{self._dqk}-dv{self._dv}"
    ...
    def generate_user_embeddings(
        ...
        user_embeddings = user_embeddings + self.position_embedding

        float_dtype = user_embeddings.dtype
        user_embeddings, cached_states = self._pcrec(
            past_lengths=past_lengths,
            x=user_embeddings,
    ...
    def forward(
        ...
        encoded_embeddings, _ = self.generate_user_embeddings(
            past_lengths=past_lengths,
            past_ids=past_ids,
            past_embeddings=past_embeddings, #torch.Size([128, 211, 50])
            past_payloads=past_payloads,
        )
    ...
    def _encode(
        ...
        past_payloads: Dict[str, torch.Tensor],
        delta_x_offsets: Optional[Tuple[torch.Tensor, torch.Tensor]],
        cache: Optional[List[pcrecCacheState]],
        return_cache_states: bool,
    ) -> Union[torch.Tensor, Tuple[torch.Tensor, List[pcrecCacheState]]]:
        """
        Args:
            past_lengths: (B,) x int64.
