...
'''def compute_kl_divergence(predictions, targets):
    #计算KL散度
...
def train_fn(
    ...
    for epoch in range(num_epochs):
        ...
        for eval_iter, row in enumerate(iter(eval_data_loader)):
            seq_features, target_ids, target_ratings = movielens_seq_features_from_row(
                row, device=device, max_output_length=gr_output_length + 1, train_flag = False,
            )
            B = target_ids.shape[0]
            for i in range(B-1):
                optimizer.zero_grad()
                single_seq_features = SequentialFeatures(
                    past_ids=seq_features.past_ids[i:i+1],
                    past_lengths=seq_features.past_lengths[i:i+1],
                    past_embeddings=None,
                    past_payloads={k: v[i:i+1] for k, v in seq_features.past_payloads.items()}
                )
                single_target_id = target_ids[i:i+1]
                past_lengths = single_seq_features.past_lengths
                assert single_seq_features.past_lengths.item() > 0, "past_lengths must be greater than 0"
                #single_seq_features.past_ids[0,past_lengths] = single_target_id
                past_embeddings=model.module.get_item_embeddings(single_seq_features.past_ids)
                model_output = model(
                    past_lengths=past_lengths,
                    past_ids=single_seq_features.past_ids,
                    past_embeddings=past_embeddings,
                    past_payloads=single_seq_features.past_payloads,
                )
                kl_loss = F.kl_div(
                    F.log_softmax(model_output[:, :past_lengths-1, :], dim=-1),
                    F.softmax(past_embeddings[:, 1:past_lengths, :], dim=-1),
                    reduction='batchmean'
                )
                #optimizer.zero_grad()
                kl_loss.backward()
                ...
                eval_dict = eval_metrics_v2_from_tensors(
                    ...
                    model.module,
                    single_seq_features,
                    target_ids=single_target_id,
                    target_ratings=target_ratings[i:i+1],
                    user_max_batch_size=eval_user_max_batch_size,
                    dtype=torch.bfloat16 if main_module_bf16 else None,
                )
        ...
        hr_500 = _avg(eval_dict_all["hr@500"], world_size=world_size)
        hr_1000 = _avg(eval_dict_all["hr@1000"], world_size=world_size)
        ...
        logging.info(
            ...
            f"NDCG@1 {ndcg_1:.4f}, NDCG@10 {ndcg_10:.4f}, NDCG@50 {ndcg_50:.4f}, NDCG@100 {ndcg_100:.4f}, NDCG@200 {ndcg_200:.4f}, HR@1 {hr_1:.4f}, HR@10 {hr_10:.4f}, HR@50 {hr_50:.4f}, HR@100 {hr_100:.4f}, HR@200 {hr_200:.4f}, HR@500 {hr_500:.4f}, HR@1000 {hr_1000:.4f}, MRR {mrr:.4f}"
        )
        last_training_time = time.time()
